...
def main(stdscr):
    ...
    if readsql:
        ...
        bigram.train_from_messages(messages)
        for row in messages:
            text = row[0]
    ...
    while True:
        ...
        if key == ord(' '): #space
            prev_word = word
            word = ""
            predictions = bigram.predict(prev_word, 1)
